...
class LatteI2V(ModelMixin, ConfigMixin):
    ...
    def forward(
            ...
            encoder_hidden_states_ip: Optional[torch.Tensor] = None,
        ...
        if self.is_input_patches:  # here
            ...
            if self.image_cross_attention:
                hidden_states_concat = rearrange(hidden_states_concat, 'b c f h w -> (b f) c h w').contiguous()
                # hidden_states_concat = self.pos_embed(hidden_states_concat.to(self.dtype))  # alrady add positional embeddings # [34, 1024, 1152]
                hidden_states = torch.cat([hidden_states, hidden_states_concat], dim=1)
        ...
        if self.caption_projection is not None:
            ...
            if use_image_num != 0 and self.training:
                ...
                if self.image_cross_attention:
                    # encoder_hidden_states_ip_video = encoder_hidden_states_ip[:, :1, ...]
                    # encoder_hidden_states_ip_video = repeat(encoder_hidden_states_ip_video, 'b 1 t d -> b (1 f) t d', f=frame).contiguous()
                    # encoder_hidden_states_ip_image = encoder_hidden_states_ip[:, 1:, ...]
                    # encoder_hidden_states_ip = torch.cat([encoder_hidden_states_ip_video, encoder_hidden_states_ip_image], dim=1)
                    encoder_hidden_states_ip_spatial = rearrange(encoder_hidden_states_ip, 'b f t d -> (b f) t d').contiguous()
                    # encoder_hidden_states_ip = rearrange(encoder_hidden_states_ip, 'b (f l) c -> (b f) l c', f=frame).contiguous()
                    encoder_hidden_states_spatial=torch.cat([encoder_hidden_states_spatial, encoder_hidden_states_ip_spatial], dim=1)
...
# depth = num_layers * 2
def LatteI2V_XL_122(**kwargs):
    return LatteI2V(num_layers=28, attention_head_dim=72, num_attention_heads=16, patch_size_t=1, patch_size=2,
                    norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=1152, **kwargs)
def LatteI2V_D64_XL_122(**kwargs):
    return LatteI2V(num_layers=28, attention_head_dim=64, num_attention_heads=18, patch_size_t=1, patch_size=2,
                    norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=1152, **kwargs)

Latte_i2v_models = {
    "LatteI2V-XL/122": LatteI2V_XL_122,
    "LatteI2V-D64-XL/122": LatteI2V_D64_XL_122,
}
