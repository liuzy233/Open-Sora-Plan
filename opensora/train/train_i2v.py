...
from opensora.models.diffusion.latte.modeling_latte_i2v import LatteI2V
from opensora.models.text_encoder import get_text_enc, get_text_warpper
from opensora.models.image_encoder import get_image_enc, get_image_warpper
...
def main(args):
    ...
    image_enc = get_image_warpper(args.image_encoder_name)(args).eval()
    ...
    model = Diffusion_models[args.model](
        ...
        image_cross_attention=args.image_conditon,
        image_cross_attention_scale_learnable=args.image_cross_attention_scale_learnable,
    )
    ...
    model.pos_embed.proj = nn.Conv2d(
        ae_channel_config[args.ae] * 2, model.num_attention_heads * model.attention_head_dim, kernel_size=(model.patch_size, model.patch_size), stride=model.patch_size, bias=True
    )
    ...
    image_enc.to(accelerator.device, dtype=weight_dtype)
    # image_enc.to(accelerator.device)

    # Create EMA for the unet.
    if args.use_ema: # False
        ema_model = deepcopy(model)
        ema_model = EMAModel(ema_model.parameters(), model_cls=LatteI2V, model_config=ema_model.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "model_ema"), LatteI2V)
            ...
            for i in range(len(models)):
                ...
                load_model = LatteI2V.from_pretrained(input_dir, subfolder="model")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss = 0.0
        for step, (x, attn_mask, input_ids, cond_mask, video, image) in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                if args.random_uncond:
                    random_uncond = torch.rand(x.size(0), device=x.device)
                else:
                    random_uncond = torch.ones(x.size(0), device=x.device)  ## by doning so, we can get text embedding and complete img emb for inference
                
                input_mask = 1 - rearrange((random_uncond >= args.uncond_prob).float() * (random_uncond < 3 * args.uncond_prob).float(), "n -> n 1 1 1")
                ...
                with torch.no_grad():
                    ...
                    if args.image_conditon:
                        ...
                        ref_img_v = [center_crop(Image.fromarray(video[i][cond_frame_index].transpose((1, 2, 0)), mode="RGB")) for i in range(B)]
                        ref_img_i = [[center_crop(image[i][j]) for j in range(args.use_image_num)] for i in range(B)]

                        ref_img_v_emb = [image_enc(ref_img_v[i], video_input=True).unsqueeze(0) for i in range(B)] ## b l c
                        ref_img_v_emb = torch.cat(ref_img_v_emb, dim=0)
                        ref_img_v_emb = rearrange(ref_img_v_emb, 'b 1 (t l) c -> b t l c', t=17)

                        ref_img_i_emb = [image_enc(ref_img_i[i]).unsqueeze(0) for i in range(B)] ## b l c
                        ref_img_i_emb = torch.cat(ref_img_i_emb, dim=0)
                        ref_img_emb = torch.cat([ref_img_v_emb, ref_img_i_emb], dim=1)

                    # Map input images to latent space + normalize latents
                    if args.use_image_num == 0:
                        if args.image_conditon and args.conditioning_key == 'hybrid':
                            ...
                            img_cat_cond = repeat(img_cat_cond, 'b c 1 h w -> b c t h w', t=x.size(2))
                            
                        x = ae.encode(x)  # B C T H W
                    else:
                        videos, images = x[:, :, :-args.use_image_num], x[:, :, -args.use_image_num:]
                        videos_ae = ae.encode(videos)  # B C T H W
                        ...
                        x = torch.cat([videos_ae, images], dim=2)   #  b c 17+4, h, w
                ...
                model_kwargs = dict(encoder_hidden_states=cond, attention_mask=attn_mask,
                                    ...
                                    hidden_states_concat=img_cat_cond if args.image_conditon else None, 
                                    encoder_hidden_states_ip=ref_img_emb if args.image_conditon else None,
                                )
...
if __name__ == "__main__":
    ...
    # for image condition
    parser.add_argument("--image_conditon", type=bool, default=True)
    parser.add_argument("--image_cross_attention_scale_learnable", type=bool, default=True)
    parser.add_argument("--random_uncond", type=bool, default=False)
    parser.add_argument("--uncond_prob", type=float, default=0.05)
    parser.add_argument("--rand_cond_frame", type=bool, default=False)
